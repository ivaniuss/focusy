...
import { Clock, Music, Image, Volume2, Play, Pause, ChevronLeft, ChevronRight, SkipBack } from 'lucide-react'
...
export function Estudio_3d() {
  ...
  const updatePomodoroTime = () => {
    setPomodoroState(prev => {
      if (prev.timeLeft <= 1) {
        const newCyclesCompleted = prev.isBreak ? prev.cyclesCompleted : prev.cyclesCompleted + 1
        const isLongBreak = newCyclesCompleted % pomodoroSettings.longBreakInterval === 0
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (!event.target.closest('.modal') && !event.target.closest('.sidebar-button')) {
        setActiveModal(null)
      }
    }
    ...
    <div className="relative w-full h-screen bg-gray-900">
      ...
      <div className="absolute left-6 top-1/2 transform -translate-y-1/2 bg-gray-800 bg-opacity-70 backdrop-blur-sm rounded-2xl p-3">
        <div className="flex flex-col space-y-4">
          <Button variant="ghost" size="icon" onClick={() => toggleModal('gallery')} className="sidebar-button text-gray-200 hover:text-white hover:bg-gray-700 focus:ring-2 focus:ring-white">
            <Image className="h-6 w-6" />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => toggleModal('sounds')} className="sidebar-button text-gray-200 hover:text-white hover:bg-gray-700 focus:ring-2 focus:ring-white">
            <Volume2 className="h-6 w-6" />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => toggleModal('music')} className="sidebar-button text-gray-200 hover:text-white hover:bg-gray-700 focus:ring-2 focus:ring-white">
      ...
      <div className="absolute right-6 top-1/2 transform -translate-y-1/2 bg-gray-800 bg-opacity-70 backdrop-blur-sm rounded-2xl p-3">
        <Button variant="ghost" size="icon" onClick={() => toggleModal('pomodoro')} className="sidebar-button text-gray-200 hover:text-white hover:bg-gray-700 focus:ring-2 focus:ring-white">
      ...
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex space-x-16">
        <ChevronLeft onClick={() => changeScene('prev')} className="h-12 w-12 text-gray-200 cursor-pointer hover:text-white transition-colors" />
        <ChevronRight onClick={() => changeScene('next')} className="h-12 w-12 text-gray-200 cursor-pointer hover:text-white transition-colors" />
      </div>

      {/* Pomodoro Timer */}
      {pomodoroState.isActive && (
        <div className={`absolute top-6 left-1/2 transform -translate-x-1/2 bg-gray-800 bg-opacity-70 text-white text-4xl font-bold rounded-2xl px-6 py-2 ${pomodoroState.isBreak ? 'border-green-500' : 'border-red-500'} border-4`}>
      ...
      {activeModal === 'pomodoro' && (
        <div className="modal absolute right-20 top-1/2 transform -translate-y-1/2 bg-gray-800 bg-opacity-90 backdrop-blur-sm p-6 rounded-2xl shadow-lg text-white">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1">Work Duration: {pomodoroSettings.workDuration} min</label>
              <Slider 
                value={[pomodoroSettings.workDuration]} 
                onValueChange={([value]) => setPomodoroSettings(prev => ({ ...prev, workDuration: value }))}
                max={60} 
                step={1} 
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Break Duration: {pomodoroSettings.breakDuration} min</label>
              <Slider 
                value={[pomodoroSettings.breakDuration]} 
                onValueChange={([value]) => setPomodoroSettings(prev => ({ ...prev, breakDuration: value }))}
                max={30} 
                step={1} 
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Long Break Duration: {pomodoroSettings.longBreakDuration} min</label>
              <Slider 
                value={[pomodoroSettings.longBreakDuration]} 
                onValueChange={([value]) => setPomodoroSettings(prev => ({ ...prev, longBreakDuration: value }))}
                max={45} 
                step={1} 
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Long Break Interval: Every {pomodoroSettings.longBreakInterval} sessions</label>
              <Slider 
                value={[pomodoroSettings.longBreakInterval]} 
                onValueChange={([value]) => setPomodoroSettings(prev => ({ ...prev, longBreakInterval: value }))}
                max={8} 
                step={1} 
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {scenes.map((scene, index) => (
              <Button 
                key={scene.name} 
                variant="outline" 
                className={`h-24 w-full ${currentSceneIndex === index ? 'border-blue-500' : 'border-gray-600'} focus:ring-2 bg-gray-800 bg-opacity-70 backdrop-blur-sm`}
          ...
          <div className="space-y-4">
            {Object.entries(ambientSounds).map(([sound, { volume }]) => (
              <div key={sound} className="flex items-center justify-between">
                ...
                <Slider
                  value={[volume]}
                  onValueChange={([newVolume]) => handleAmbientSoundVolume(sound, newVolume)}
                  max={100}
                  step={1}
                  className="w-48"
                />
      ...
      {activeModal === 'music' && (
        <div className="modal absolute left-20 top-1/2 transform -translate-y-1/2 bg-gray-800 bg-opacity-90 backdrop-blur-sm p-6 rounded-2xl shadow-lg text-white">
          ...
          <div className="space-y-4">
            ...
            <Button variant="outline" className="w-full focus:ring-2 bg-gray-800 bg-opacity-70 backdrop-blur-sm" onClick={togglePlayPause}>
